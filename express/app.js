...
const getApiProxiConfig = () => {
  ...
  return {
    ...
    onProxyReq: (proxyReq, req, res) => {
      const jwtData = jwtDecode(req.headers.authorization.split(' ')[1])
...
if (isDevServer) {
  ...
  const webpackConfig = require(paths.ROOT + '/webpack.config')({ _mode: DEV })
  const webpackMiddlewareConfig = {
    publicPath: '/',
    writeToDisk: true
  }
  const webpackCompiler = webpack(webpackConfig)
  express_.use(webpackDevMiddleware(webpackCompiler, webpackMiddlewareConfig))
