...
const UploadDemos: React.FC<UploadDemosOwnProps> = (props: UploadDemosOwnProps) => {
  ...
  const fakeChunkFileUpload = async (fileName: string, chunkIndex: number, chunk: Blob, startPos: number, endPos: number): Promise<any> => {
    return await delayedPromise(1000 + (chunkIndex * 1000), { startPos, endPos, chunk }).then(res => {
  ...
  const handleUpload = () => {
    ...
    for (let i = 0; i < state.chunkCount; i++) {
      ...
      void fakeChunkFileUpload('tmp', i, chunk, startPos, endPos)
    }
  }
    ...
    <br />
      ...
      chunks: {
        state.chunksStatus.map((ck: any, i: number) => {
          return <div key={i}><span>{i} : {ck.state} : {ck.startPos} : {ck.endPos}</span></div>
        })
      }
  </div>
}
